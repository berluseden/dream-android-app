...
async function seedMuscles() {
    ...
    for (const muscle of muscles) {
        ...
        if (!doc.exists) {
            await docRef.set(Object.assign({ id: muscle.name }, muscle));
...
async function seedExercises() {
    ...
    for (const exercise of exercises) {
        ...
        if (snapshot.empty) {
            await db.collection('exercises').add(Object.assign(Object.assign({}, exercise), { video_url: '', description: '', instructions: '', created_by: null }));
...
async function seedTemplates(adminId) {
    ...
    for (const template of allTemplates) {
        ...
        if (snapshot.empty) {
            await db.collection('templates').add(Object.assign(Object.assign({}, template), { created_by: adminId, rating_avg: 0, rating_count: 0, times_used: 0, created_at: admin.firestore.FieldValue.serverTimestamp(), updated_at: admin.firestore.FieldValue.serverTimestamp() }));
...
// Main seed function
exports.seedCatalogs = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        await (0, audit_1.createAuditLog)(adminId, 'EXECUTE_SEED', 'catalogs', {
